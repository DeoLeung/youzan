  ...
  return function(err, res, data) {
    if (!data) {
      ...
      try {
        ...
        err = new Error(data);
        err.name = 'JSONParsingError';
      }
    }
    callback = callback || function() {};
    if (!err && data.error_response) {
      err = new Error(data.error_response.msg);
...
let post = function(config, method, apiParams, filePaths, filekey, callback) {
  ...
  if (filePaths && filePaths.length > 0 && filekey) {
    opts.formData = {};
    opts.formData[filekey] = [];

    filePaths.forEach(function(file_path) {
      opts.formData[filekey].push(fs.createReadStream(file_path));
    });
  }
